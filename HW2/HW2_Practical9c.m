...
for(iTime = 1:numFrames)
    ...
    for particleNum = 1:numParticles
        ...
        % motion model.
        noise = 14*randn(1,size(particles_old,2)); % more noise increases corner 'search field' but reduces coverage 'density'
        particles_new(particleNum,:) = particles_old(samples_to_propagate(particleNum),:) + noise; % as in lab
    ...
    % (EXTRA CREDIT) Edge detection - see below
    detect = edge(Im2);
    ...
    for particleNum = 1:numParticles
        ...
        if(inFrame)
            ...
            % (EXTRA CREDIT) Edge detection - reduce search space
            if(minX <= size(detect,1) && minY <= size(detect,2) && minX > 0 && minY > 0)
                weight_of_samples(particleNum) = weight_of_samples(particleNum)...
                    + 0.5*detect(minX,minY); % readjust weights
            end
        else
    ...
    weightedAve = sum([weight_of_samples weight_of_samples].*particles_new);
