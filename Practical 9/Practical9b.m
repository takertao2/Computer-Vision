...
% Load template and starting position ('pos'), which comes from frame 871.
load Template
...
for(iTime = 1:22)
    ...
    for (sampNum = 1:numParticles) 
        ...
        for (count = 1:numParticles)
            if(cum_hist_of_weights(count) > thresh)
                break;
            end;
        end;
        samples_to_propagate(sampNum) = count;
    end;
    ...
    for (particleNum = 1:numParticles)
        ...
        % motion model.
        count = samples_to_propagate(particleNum);
        noise = 10*randn(1,2);
        particles_new(particleNum,1:2) = particles_old(count,1:2) + ...
                                         particles_old(count,3:4) + noise; % add position noise
        particles_new(particleNum,3:4) = particles_old(count,3:4) + noise/2; % add velocity noise - REDUCE FOR MORE ACCURACY BUT LESS COVERAGE
